...
constexpr uint8_t kUnusedPagingMask = 0xC0; // Bits 6-7 are unused
...
void memory_paging_bank_switch(uint8_t value) {
	...
	uint8_t masked_value = value & kRAMPagingBankMask;
	memory_page_set_map(kRAMPagingBankAddr, masked_value);

	// SCR – Set to 0 to display normal screen, 1 to display screen in bank 7
	masked_value = value & kScreenPagingBankMask;
	memory_page_set_map(BANK_ADDR_FIXED_SCREEN, masked_value ? kBankScreenIndex1 : kBankScreenIndex0);
//
	// ROM – Set to 0 to page in the 128K editor, or 1 to page in the 48K BASIC ROM
	masked_value = value & kROMPagingBankMask;
	...
	masked_value = value & kROMPagingLockMask;
