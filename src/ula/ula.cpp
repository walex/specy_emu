...
void ula_read_tape(uint64_t clock_cycle, uint8_t* value) {
	...
	static bool prev_tape_audio_playing = false;

	uint64_t delta_tstates = clock_cycle - last_clock;
	last_clock = clock_cycle;

	// Track tape playing state
	bool tape_audio_playing;
	if ((tape_audio_playing = tape_audio_is_active()) == false)
		if ((tape_audio_playing = automata_port_is_waiting_for_tone(delta_tstates)) == true)
			tape_audio_next_pulses_block();

	// Reset timing when tape starts playing and this is the first port read
	if (tape_audio_playing && !prev_tape_audio_playing)
		last_clock = clock_cycle;

	prev_tape_audio_playing = tape_audio_playing;
	...
	if (tape_audio_playing && delta_tstates > MAX_TAPE_DELTA) {
		delta_tstates = MAX_TAPE_DELTA; // Clamp, don't pause
	}	

	// get audio pulses
	if (tape_audio_playing) {
